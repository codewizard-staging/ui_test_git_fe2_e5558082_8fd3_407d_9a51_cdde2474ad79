...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, prodImages, petId;
        ...
        // Add Or Update Pet
        rslt = await Support.AddOrUpdatePet(pet, dropDownOptions, []);
        if (rslt.status) {
            petId = rslt.id;
        } else { return; }
        ...
        for (let i = 0; i < mapItems.length; i++) {
            ...
            if (!Helper.IsJSONEmpty(navItem) && Helper.IsNullValue(navItem.value)) {
                ...
                if (rslt.status) {
                    ...
                    rslt = await Support.AddOrUpdatePet(data, dropDownOptions);
        ...
        global.AlertPopup("success", "Pet is created successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
