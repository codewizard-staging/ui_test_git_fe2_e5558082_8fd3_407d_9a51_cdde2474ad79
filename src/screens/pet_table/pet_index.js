...
const columns = [
    { headerName: "PetId", field: "PetId", flex: 1 },
    { headerName: "PetName", field: "PetName", flex: 1 },
    { headerName: "Breed", field: "Breed", flex: 1 },
];
...
const Component = (props) => {
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetPetSingle({ PetId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Pets/edit/${id}`;
        if (type === 'view') _route = `/Pets/view/${id}`;
    ...
    return (
        ...
        <>
            <Container {...props}>
                ...
                <Box style={{ width: '100%' }}>
                    <DataTable keyId={'PetId'} columns={columns} rowsCount={rowsCount} rows={rows} sortBy={sortBy} pageInfo={pageInfo} onActionClicked={OnActionClicked}
